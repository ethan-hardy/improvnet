...
def getNoteGroup(pitch, duration):
    return [ { 'pitch': pitch, 'isNoteBeginning': ind == 0 } for ind in range(0, duration)]

def createQuantizedList(track, ticksPerBeat):
...
def readFile():
  file = MidiFile('ladybird.mid')

  notes = createQuantizedList(file.tracks[0], file.ticks_per_beat)
